...
- **凭据健康诊断**: 展示健康状态、最近错误、事件历史、本地 RPM 限制和冷却信息
- **凭据恢复操作**: 支持重置失败、真实发消息验活、清除冷却、按风险恢复凭据
- **模型兼容调度**: 按凭据支持的模型筛选候选凭据，减少 `INVALID_MODEL_ID` 重复失败
- **余额可视化**: Admin UI 显示套餐、使用量、剩余额度和下次重置时间
